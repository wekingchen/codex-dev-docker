...
公开默认镜像只包含Codex，因此 `claude` 用法需要先切换到personal双CLI镜像。
...
docker compose run --rm codex claude
```
...
脚本要求显式传入实际 Docker volume 名并输入 `DELETE <volume-name>`。它会先停止本项目容器、检查是否仍有其他容器引用目标、直接删除并再次复核。不要对仍需保留的登录状态或 runtime 执行该操作；personal双CLI用户会同时失去Codex和Claude Code认证、配置与历史。
...
Codex登录状态保存在 `/home/dev/.codex`。personal双CLI用户可另外运行：
...
private dual-CLI使用独立的 `docker-personal.yml`、`cleanup-personal-ghcr.yml` 和 `supply-chain-audit-personal.yml`。它不会改变公开package；完整bootstrap与promotion顺序见 [`PERSONAL-DUAL-CLI.md`](PERSONAL-DUAL-CLI.md)。
...
personal cleanup使用独立workflow，只删除保留数量之外、仍然只有candidate标签的未提升版本；日期、commit、latest、未知标签和全部untagged版本永久保护。真正删除还需要独立的 `GHCR_PRIVATE_CLEANUP_TOKEN`。
