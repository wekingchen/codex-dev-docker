...
- Node.js 固定当前 Node 24 LTS 的精确官方 release，并分别校验 Linux x64、arm64 tarball 的 SHA-256；npm另行固定官方registry tarball及SHA-512，确保内置 `tar` 依赖已修复已知CRITICAL漏洞；项目需要其他版本时仍可由 mise 覆盖。
...
- `.github/workflows/supply-chain-audit.yml` 每周只读检查 Actions SHA、Ubuntu digest、mise asset、Node.js 24 LTS、npm官方tarball/内置依赖和 Trivy 版本漂移，不自动修改仓库或 registry。
...
Dependabot继续更新 GitHub Actions 和 Docker tag；同 tag Ubuntu digest、mise、Node.js 24 LTS、npm和 Trivy binary的漂移由审计 workflow提醒后人工更新，并重新通过完整双架构 candidate流程。
